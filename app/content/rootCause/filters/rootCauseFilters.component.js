...
class RootCauseFiltersCtrl extends FiltersController {
    ...
    removeFilter(index) {
        ...
        this.$rootScope.$broadcast('rootCause.filter.deleted', filter);
    }
    ...
    applyFilter(index) {
        ...
        if (this.isAdded(index)) {
            ...
            this.$rootScope.$broadcast('rootCause.filter.updated', filter);
        } else {
            // Add new filter and equip it with new ID
            filterData.id = this.getNextId();
            filter.id = filterData.id;
            this.$rootScope.$broadcast('rootCause.filter.created', filter);
        }
...
truedashApp.component('appRootCauseFilters', {
    controller: RootCauseFiltersCtrl,
    templateUrl: 'content/rootCause/filters/rootCauseFilters.html',
    bindings: {
        existingFilters: '=',
        table: '=',
    }
