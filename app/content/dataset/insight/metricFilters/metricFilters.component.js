...
import {FilterDTO} from '../../../metricBuilder/metricFilters/dto/FilterDTO';
...
export class MetricFiltersController extends FiltersController {
    ...
    initFilters() {
        ...
        return this.metricFilters.init().catch(error => {
            console.error(error);
        }).then(() => {
    ...
    initAvailableColumns(useCache = true) {
        return this.metricFilters.loadAvailableColumns(useCache)
...
truedashApp.component('appMetricFilters', {
    controller: MetricFiltersController,
    templateUrl: 'content/dataset/insight/metricFilters/metricFilters.html',
    bindings: {
        resolve: '<',
        close: '&',
        dismiss: '&'
