...
class KeyDriversFiltersCtrl extends FiltersController {
    ...
    removeFilter(index) {
        ...
        this.$rootScope.$broadcast('keyDrivers.filter.deleted', filter);
    }
    ...
    applyFilter(index) {
        ...
        if (this.isAdded(index)) {
            ...
            this.$rootScope.$broadcast('keyDrivers.filter.updated', filter);
        } else {
            // Add new filter and equip it with new ID
            filterData.id = this.getNextId();
            filter.id = filterData.id;
            this.$rootScope.$broadcast('keyDrivers.filter.created', filter);
        }
...
truedashApp.component('appKeyDriversFilters', {
    controller: KeyDriversFiltersCtrl,
    templateUrl: 'content/machineLearning/keyDrivers/filters/keyDriversFilters.html',
    bindings: {
        existingFilters: '=',
        table: '=',
    }
